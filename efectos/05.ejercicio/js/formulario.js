...
===============================================>>>>>*/
var pf= {
  entrada: document.querySelectorAll("input.validar"),
  valor: null,
...
/*=============================================>>>>>
  EL OBJETO DE METODOS DEL FORMULARIO
===============================================>>>>>*/

var mf = {
  inicioFormulario:function(){
    for (var i = 0; i < pf.entrada.length; i++) {

      pf.entrada[i].addEventListener("focus",mf.enfoco);
      pf.entrada[i].addEventListener("blur",mf.salirfoco);
      pf.entrada[i].addEventListener("change",mf.validar)
  ...
  enfoco:function(input){
    ...
    if(pf.valor==""){
      document.querySelector("#"+input.target.id).style.background="rgba(225,225,0,.5)";
  ...
  varlidarFormulario:function(){
    pf.validarTermino=document.querySelector("#terminos").checked;
     console.log(pf.validarTermino);
