...
var mg ={
  ...
  inicioGaleria:function(){
      ...
      for(var i=0; i<pg.imgGaleria.length;i++){
        pg.imgGaleria[i].addEventListener("click",mg.capturaImg);
      }
  ...
  lightbox:function(rutaImg){
    ...
    if(pg.animacionGaleria=="slide"){
      ...
      pg.modal.style.opacite="0";
      setTimeout(function(){
        pg.modal.style.transition=".5s opacite ease"
        pg.modal.style.top="50%";
        pg.modal.style.left="48%";
        pg.modal.style.opacite="1";
        pg.modal.style.marginLeft= -pg.modal.childNodes[0].width/2+"px";
    ...
    pg.modal.childNodes[1].style.borderRedius="0px 0px 0px 5px";


    pg.modal.childNodes[1].addEventListener("click",mg.borarlightbox);


  },
  borarlightbox:function(){
